...
def main(path_output):
    ...
    model.add_atomic_energy("Br", reference.compute_atomic_energy("Br", box_size=6))
...
def restart(path_output):
    ...
    model, walkers, data_train, data_valid = load_state(path_output, '4')
    data_train, data_valid = learning.run(
            model=model,
            reference=reference,
            walkers=walkers,
